...
void updateDisplay(int menuNum)
{
  ...
  {
    const uint8_t *bitmap = (menuNum == 0) ? bitmap_raw_repeater : (menuNum == 1) ? bitmap_exit
                                                                                  : bitmap_access_point;
    display.drawBitmap(0, 0, bitmap, 128, 64, WHITE);
  }
...
void spot()
{
  ...
  while (true)
  {
    ...
    {
      if (btn.isSingle())
        frequency += step_single;
      else if (btn.isDouble())
        frequency += step_double;
      else if (btn.isTriple())
        frequency += step_triple;
      else if (btn.isHold())
        frequency += step_hold;
      else
        return;
...
void rec_play()
{
  ...
  auto handleFrequencyAdjustment = [&]()
  {
    ...
    {
      if (btn.isSingle())
        rec_frequency += step_single;
      else if (btn.isDouble())
        rec_frequency += step_double;
      else if (btn.isTriple())
        rec_frequency += step_triple;
      else if (btn.isHold())
        rec_frequency += step_hold;
      else
        return false;
  ...
  auto handleSamplingAdjustment = [&]()
  {
    ...
    {
      if (btn.isSingle())
        rec_sampling += step_single;
      else if (btn.isDouble())
        rec_sampling += step_double;
      else if (btn.isTriple())
        rec_sampling += step_triple;
      else if (btn.isHold())
        rec_sampling += step_hold;
      else
        return false;
...
void range()
{
  ...
  while (true)
  {
    ...
    auto handleButton = [&](GButton &btn, float step_single, float step_double, float step_hold, bool temp)
    {
      ...
      {
        if (start_frequency < 300.00)
          start_frequency = 300.00;
        if (start_frequency > 928.00 - step)
          start_frequency = 928.00 - step;
        if (stop_frequency < start_frequency + step)
          stop_frequency = start_frequency + step;
      ...
      {
        if (btn.isSingle())
          start_frequency += step_single;
        else if (btn.isDouble())
          start_frequency += step_double;
        else if (btn.isHold())
          start_frequency += step_hold;
        else
          return;
      }
      else
      {
        if (btn.isSingle())
          stop_frequency += step_single;
        else if (btn.isDouble())
          stop_frequency += step_double;
        else if (btn.isHold())
          stop_frequency += step_hold;
        else
          return;
      }
...
void setup()
{
  ...
  if (access_point == 0)
  {
    ...
    registerRoute("/setting_logo", []()
                  { sendHtmlAndExecute(html_logo_setings); });
    registerRoute("/setting_access_point", []()
                  { sendHtmlAndExecute(html_access_point_settings); });
    ...
    registerRoute("/logo_on", []()
                  { storeEEPROMAndSet(0, 0, logo, html); });
    registerRoute("/logo_off", []()
                  { storeEEPROMAndSet(0, 1, logo, html); });
    registerRoute("/access_point_off", []()
                  { AccessPointHandler(1); });
